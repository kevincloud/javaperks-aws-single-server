...
curl \
    --header "X-Vault-Token: $VAULT_TOKEN" \
    --request POST \
    --data "{ \"db_name\": \"custapidb\", \"creation_statements\": [\"CREATE USER '{{name}}'@'%' IDENTIFIED BY '{{password}}';\", \"GRANT ALL PRIVILEGES ON $MYSQL_DB.* TO '{{name}}'@'%';\", \"FLUSH PRIVILEGES;\"], \"default_ttl\": \"1h\", \"max_ttl\": \"24h\" }" \
